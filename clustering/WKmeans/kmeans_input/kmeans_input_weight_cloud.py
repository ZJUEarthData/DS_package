# -*- coding: utf-8 -*-
"""
Created on Thu Oct 15 21:04:25 2020
...
def main():
    ...
    n_iters = 10
    n_clusters = 4
    centroids = plus_plus(x_data, n_clusters)  # centroilds initialization
    dist = np.zeros((x_data.shape[0],n_clusters), dtype=float)

    print("initial centroids are:",centroids)

    w = pd.read_excel(FEATURE_FILE_PATH)  # input weight from weight.xlsx flie
    w = np.array(w)
    tol=1e-10
    count=0
    C = np.zeros(n_iters)
