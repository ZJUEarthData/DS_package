...
import joblib
import warnings
from mlp_training_cloud import get_data,data_processt
import pandas as pd
...
#    base = os.getcwd()
    ...
    svm_test = joblib.load(INPUT_MODEL+'test.pkl')             # call the pre-trained model
    print(svm_test)
    ...
    if X.isnull().any().any():
        raise ValueError("Missing data in feature file")
    # data pre-process
    X_ = data_process(X,CHOICE)

    # data prediction and results visualization 
    y_pred = svm_test.predict(X_)
