...
def write_wrong_label(classifier, data_list, label_list,filename):                #  output wrong classification samples
    ...
    wrong_sheet.write(0, 0, 'Total category errors')
...
def plot_learning_curve(model,data,labels):
    ...
    plt.fill_between(train_sizes, train_scores_mean - train_scores_std, train_scores_mean + train_scores_std, alpha=0.1,
                     color='r')
    #plt.fill_ The between() function fills the space of the upper and lower variances of the average model accuracy with colors.
...
def main():
    ...
    # save model
    joblib.dump(svm_best_model.best_estimator_, "./test.pkl")
    ...
    plot_learning_curve(svm_best_model.best_estimator_, data, labels)
    #  Output prediction error samples
    write_wrong_label(svm_best_model, data, labels, 'wrong.xlsx')
