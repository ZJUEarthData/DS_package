...
def fit(x_data, y_label):       # training
...
def l_curve(train_sizes, train_scores, test_scores):       # Draw a learning curve
...
def main()
    df = pd.read_csv("test_result_newxk2000_4簇.csv")    # View the value of the classification tag after clustering using Kmeans, two types
    df['km_clustering_label'].hist()

    stratified(df)

    orig_data = pd.read_csv('test4_result.csv')         # Read the sampling results
    # orig_data = pd.read_excel('test_newxk2000.xlsx')
    orig_data.dropna(inplace=True)                      # Deletes rows that contain missing values
    # orig_data
    x_orig_data = orig_data.drop('TRUE VALUE', axis=1)
    y_orig_label = orig_data['TRUE VALUE']
    y_label = y_orig_label.replace(-1, 0)

    process = preprocessing.StandardScaler()  # The normalized
    ...
    print('Accuracy: %.3f' % accuracy_score(y_test, y_test_pred))       # Output prediction accuracy
